...
const GallerySlider = (props: IGallery) => {
  ...
  return (
    <div className="slider">
      ...
      >
        {props.images.map((image: Image, index: number) => (
          <li onMouseEnter={() => setDotsVisible(true)} onMouseLeave={() => handleDotsVisibility(false)} key={index} className="slider-item">
            <img src={image.imageUrl} alt={`slide ${index}`} />
          </li>
        ))}
      </ul>
      <button style={{opacity:`${dotsVisible ? '1' : '0'}`,transition:'1s'}} className="slider-button slider-button-previous" onClick={handlePreviousClick}>
        {'<'}
      </button>
      <button style={{opacity:`${dotsVisible ? '1' : '0'}`,transition:'1s'}} className="slider-button slider-button-next" onClick={handleNextClick}>
        {'>'}
      </button>
      {renderDots()}
    </div>
