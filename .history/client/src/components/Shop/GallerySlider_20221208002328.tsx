...
const GallerySlider = (props: IGallery) => {
  ...
  return (
    <div className="slider">
      ...
      >
        {props.images.map((image: Image, index: number) => (
          <li key={index} className="slider-item">
            <div className="slidet-item__open">
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
      ...
      <button onMouseEnter={() => setDotsVisible(true)} style={{opacity:`${dotsVisible ? '1' : '0'}`,transition:'1s'}} className="slider-button slider-button-previous" onClick={handlePreviousClick}>
        {'<'}
      </button>
      <button onMouseEnter={() => setDotsVisible(true)} style={{opacity:`${dotsVisible ? '1' : '0'}`,transition:'1s'}} className="slider-button slider-button-next" onClick={handleNextClick}>
        {'>'}
      </button>
      {renderDots()}
    </div>
